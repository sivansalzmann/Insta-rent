...
export default function RenterPage(props) {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/api/users/${props.location.renterId}`)
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/api/renterDeatils/${props.location.renterId}`)
      .then(response => response.json())
      .then(result =>  {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/api/assets?RenterId=${props.location.renterId}`)
      .then(response => response.json())
      .then(result => {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/api/messages?RenterId=${props.location.renterId}`)
      .then(response => response.json())
      .then(result =>  {
